...
def trace_blocks(n0: int, max_blocks: int = 4000000):
    n = odd_start(n0)
    rows = []
    total_debt = 0
...
def analyze_start(n0: int):
    ...
    summary = {
        ...
        "min_post_chain_recovery_surplus": min(
            [row["post_chain_recovery_surplus"] for row in out if row["post_chain_recovery_surplus"] is not None],
            default=None,
        ),
        "max_hardness_score": max(
            [row["hardness_score"] for row in out if math.isfinite(row["hardness_score"])],
            default=0,
        ),
    }
...
def main():
    ...
    print("=" * 80)
    print("COLLATZ-NATIVE-MATH v0.9")
    print("=" * 80)
    print("Analyzing hard recovery cases.")
    print("=" * 80)
    ...
    for n0 in SAMPLES:
        ...
        if summary["min_post_chain_recovery_surplus"] is None:
            print("min_post_chain_recovery_surplus: None")
        else:
            print(f"min_post_chain_recovery_surplus: {summary['min_post_chain_recovery_surplus']:.6f}")
        print(f"max_hardness_score: {summary['max_hardness_score']:.6f}")

        if rows:
            recovered = [r for r in rows if r["post_chain_recovery_found"]]
            if recovered:
                hard = max(recovered, key=lambda r: r["hardness_score"])
                print(
                    "hardest_case: "
                    f"chain_end={hard['chain_end_block']} "
                    f"distance={hard['post_chain_recovery_distance_blocks']} "
                    f"surplus={hard['post_chain_recovery_surplus']:.6f} "
                    f"deficit={hard['chain_deficit_below_threshold']:.6f} "
                    f"hardness={hard['hardness_score']:.6f}"
                )
    ...
    recovered = [row for row in all_rows if row["post_chain_recovery_found"]]
    unrecovered = [row for row in all_rows if row["post_chain_unrecovered"]]
    ...
    global_summary = {
        ...
        "post_chain_recovered_episodes": len(recovered),
        "post_chain_unrecovered_episodes": len(unrecovered),
        "all_post_chain_episodes_recovered": len(unrecovered) == 0,
        "top_by_hardness_score": top_n(recovered, "hardness_score", 10, True),
        "top_by_recovery_distance": top_n(recovered, "post_chain_recovery_distance_blocks", 10, True),
        "top_by_smallest_surplus": top_n(recovered, "post_chain_recovery_surplus", 10, False),
        "top_by_chain_deficit": top_n(recovered, "chain_deficit_below_threshold", 10, True),
        "top_by_recovery_gap": top_n(
            [r for r in recovered if r["recovery_distance_gap"] is not None],
            "recovery_distance_gap",
            10,
            True,
        ),
        "max_hardness_score": max([r["hardness_score"] for r in recovered], default=None),
        "max_post_chain_recovery_distance": max([r["post_chain_recovery_distance_blocks"] for r in recovered], default=None),
        "min_post_chain_recovery_surplus": min([r["post_chain_recovery_surplus"] for r in recovered], default=None),
        "max_chain_deficit": max([r["chain_deficit_below_threshold"] for r in recovered], default=None),
    ...
    print(f"post_chain_recovered_episodes: {len(recovered)}")
    print(f"post_chain_unrecovered_episodes: {len(unrecovered)}")
    print(f"all_post_chain_episodes_recovered: {str(len(unrecovered) == 0).lower()}")

    if recovered:
        hard = max(recovered, key=lambda r: r["hardness_score"])
        long = max(recovered, key=lambda r: r["post_chain_recovery_distance_blocks"])
        tight = min(recovered, key=lambda r: r["post_chain_recovery_surplus"])
        ...
        print(
            "longest recovery: "
            f"n0={long['n0']} "
            f"distance={long['post_chain_recovery_distance_blocks']} "
            f"surplus={long['post_chain_recovery_surplus']:.6f}"
        )
        print(
            "tightest surplus: "
            f"n0={tight['n0']} "
            f"distance={tight['post_chain_recovery_distance_blocks']} "
            f"surplus={tight['post_chain_recovery_surplus']:.6f}"
        )

    print("=" * 80)
    print(f"Wrote hard recovery rows to: {RESULTS_PATH.relative_to(ROOT)}")
