...
MODULE_PATH = ROOT / "examples" / "build_response_time_invariance_audit_v0.py"


def load_module():
    module_name = "build_response_time_invariance_audit_v0"
...
def test_measure_seed_has_response_fields():
    m = load_module()
    ...
    assert "has_immediate_response" in record
...
def test_summarize_records_basic():
    ...
    records = [
        {
            "debt_peak": 1.0,
            "response_delay": 1,
            "has_immediate_response": True,
            "post_peak_release_mass_h": 2.0,
        },
        {
            "debt_peak": 2.0,
            "response_delay": 2,
            "has_immediate_response": False,
            "post_peak_release_mass_h": 3.0,
        },
    ]
    ...
    assert summary["p_delay_1"] == 0.5
    assert summary["p_delay_le_2"] == 1.0
    assert summary["no_response_rate"] == 0.0
...
def test_run_block_small():
    ...
    assert "median_response_delay" in result["aggregate"]
    assert "p_delay_1" in result["aggregate"]


def test_build_summary_has_boundary_and_timing():
    m = load_module()
    ...
    assert "timing_status" in summary["timing_summary"]
