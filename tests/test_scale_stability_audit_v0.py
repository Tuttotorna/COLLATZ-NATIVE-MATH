...
MODULE_PATH = ROOT / "examples" / "build_scale_stability_audit_v0.py"


def load_module():
    module_name = "build_scale_stability_audit_v0"
...
def test_measure_seed_has_scale_fields():
    m = load_module()
...
def test_run_config_small():
    ...
    assert result["counts"]["odd_seeds_measured"] == 11
    assert "debt_peak_vs_post_peak_release_mass_h_pearson" in result["aggregate"]


def test_build_summary_has_boundary_and_configs():
    m = load_module()
    ...
    assert summary["stability"]["configs_tested"] == 2
